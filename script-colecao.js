...
async function run(){ await typeTo(brandEl, BRAND_TEXT); await new Promise(r=>setTimeout(r,HOLD_AFTER_TYPE)); const el=document.querySelector('.brand-type'); el.classList.add('flicker'); await new Promise(r=>setTimeout(r,5000)); el.classList.remove('flicker'); brandEl.textContent=''; await new Promise(r=>setTimeout(r,LOOP_GAP)); run(); } run();
