...
// Mapear links oficiais via data-link
const LINKS = {
...
// Digitação + flicker sincronizados
...
async function loop(){ await typeTo(brandEl, BRAND_TEXT); await typeTo(heroEl, HERO_TEXT); await new Promise(r=>setTimeout(r,HOLD)); await flickerOnce('.brand-type'); await flickerOnce('.hero-type'); brandEl.textContent=''; heroEl.textContent=''; await new Promise(r=>setTimeout(r,GAP)); loop(); }
loop();
